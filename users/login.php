 ...
 if($email !== "" && $password !==""){
    ...
    if ($row == 1) {
        ...
        // FILTER_VALIDATE_EMAIL — Valide une adresse de courriel (Filtre de validation)
            ...
            }else{header('location: ../login.utilisateur.php?login_err=password');
                echo 'invalide';
            }
            }
            }else {             header('location: ../login.utilisateur.php?login_err=email');
            }
            }else {
                header('location: ../login.utilisateur.php?login_err=already');
            }
        }else {header('location: ../login.utilisateur.php');
    }
